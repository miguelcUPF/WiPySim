...
def objective(trial, training_scenarios, strategy, rl_mode):
    # Sample parameters according to strategy
    ...
    if strategy in ["sw_linucb", "linucb"]:
        ...
        if strategy == "sw_linucb":
            window_size = trial.suggest_int("window_size", 0, 84)
            agents_settings["window_size"] = None if window_size == -1 else window_size
    ...
    for step, scenario in enumerate(training_scenarios):
        cmd = [
            sys.executable,
            "tests/trial_runner.py",
            str(trial.number),
            json.dumps(agents_settings),
            strategy,
            str(rl_mode),
            json.dumps(scenario),
            str(step),
        ...
        try:
            ...
            if trial.should_prune():
                print(
                    f"Trial {trial.number} pruned at step {step} with intermediate result {result}"
                )
                raise optuna.TrialPruned()

        except subprocess.CalledProcessError as e:
            print(f"Trial {trial.number} failed at step {step} with error:\n{e.stderr}")
            raise optuna.TrialPruned()

    return sum(results) / len(results) if results else float("inf")
