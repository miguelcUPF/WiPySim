...
from src.utils.messages import STARTING_EXECUTION_MSG, EXECUTION_TERMINATED_MSG, PRESS_TO_CONTINUE_MSG, STARTING_SIMULATION_MSG, SIMULATION_TERMINATED_MSG
...
def validate_params():
    ...
    for name, value in positive_int_params.items():
        if not isinstance(value, int) or value <= 0:
            logger.critical(
                f"Invalid {name}: {value}. It must be a positive integer.")
    ...
    for name, value in float_or_int_params.items():
        if not isinstance(value, (int, float)):
            logger.critical(
                f"Invalid {name}: {value}. It must be an integer or float.")

    if not isinstance(sparams.MPDU_ERROR_PROBABILITY, (int, float)) or not (0 <= sparams.MPDU_ERROR_PROBABILITY <= 1):
        logger.critical(
...
def validate_config():
    ...
    if cfg.SEED is not None:
        if not isinstance(cfg.SEED, int):
            logger.critical(
                f"Invalid SEED: {cfg.SEED}. It must be an integer.")
    ...
    for name, value in bool_settings.items():
        if not isinstance(value, bool):
            logger.critical(
                f"Invalid {name}: '{value}'. It must be a boolean.")
    ...
    valid_modules = ["NETWORK", "NODE", "GEN", "LOAD", "APP",
                     "MAC", "PHY", "MEDIUM", "CHANNEL"]
    valid_log_levels = ["HEADER", "DEBUG", "INFO", "WARNING", "ALL"]

    for module, levels in cfg.EXCLUDED_LOGS.items():
        if module not in valid_modules:
            logger.warning(
                f"Invalid module name: '{module}' in EXCLUDED_LOGS.")
    ...
    for bss in cfg.BSSs:
        ...
        if len(bss["stas"]) == 0:
            logger.critical(
                f"BSS {bss['id']} does not have any STAs.")
        ...
        for traffic_flow in bss["traffic_flows"]:
            ...
            if "model" in traffic_flow:
                ...
                if model.get("start_time_us", None) is not None and not model["start_time_us"].is_integer():
                    logger.critical(
                        f"Invalid start_time_us: {model['start_time_us']} in BSS {bss['id']}. It must be an integer.")
                if model.get("end_time_us", None) is not None and not model["end_time_us"].is_integer():
                    logger.critical(
                        f"Invalid end_time_us: {model['end_time_us']} in BSS {bss['id']}. It must be an integer.")
                if model.get("traffic_load_kbps", None) is not None and not model["traffic_load_kbps"].is_integer():
                    logger.critical(
                        f"Invalid traffic_load_kbps: {model['traffic_load_kbps']} in BSS {bss['id']}. It must be an integer.")
                if model.get("max_packet_size_bytes", None) is not None and not isinstance(model["max_packet_size_bytes"], int):
                    logger.critical(
                        f"Invalid max_packet_size_bytes: {model['max_packet_size_bytes']} in BSS {bss['id']}. It must be an integer.")
                if model.get("burst_size_pkts", None) is not None and model["name"] == "Bursty" and not isinstance(model["burst_size_pkts"], int):
                    logger.warning(
                        f"Invalid burst_size_pkts: {model['burst_size_pkts']} in BSS {bss['id']}. It must be an integer.")
                if model.get("avg_inter_packet_time_us", None) is not None and model["name"] in ["Bursty", "VR"] and not isinstance(model["avg_inter_packet_time_us"], int):
                    logger.warning(
                        f"Invalid avg_inter_packet_time_us: {model['avg_inter_packet_time_us']} in BSS {bss['id']}. It must be an integer.")
                if model.get("fps", None) is not None and model["name"] == "VR" and not isinstance(model["fps"], int):
                    logger.warning(
            ...
            if "file" in traffic_flow:
                ...
                if file.get("start_time_us", None) is not None and not file["start_time_us"].is_integer():
                    logger.critical(
                        f"Invalid start_time_us: {file['start_time_us']} in BSS {bss['id']}. It must be an integer.")
                if file.get("end_time_us", None) is not None and not file["end_time_us"].is_integer():
                    logger.critical(
...
def warn_overwriting_enabled_paths():
    ...
    enabled_settings = [name for name,
                        enabled in path_settings.items() if enabled]
...
if __name__ == "__main__":
    ...
    initialize_network(env, cfg.BSS_CONFIG)
